# Phase 1 API Contracts

Owner: Architecture Team
Last Updated: 2026-04-22
Change Summary: Added the backend session-bootstrap, profile, and client-shell contracts for the current Phase 1 web entry flow, authenticated home-feed landing, and the live social flow for stories, vibes, search, follows, public profiles, reposts, likers, and responsive comment interactions.

## Contracts

- `IDENTITY_BOOTSTRAP_API.md`
- `IDENTITY_SESSION_BOOTSTRAP_API.md`
- `IDENTITY_ME_API.md`
- `IDENTITY_PROFILE_API.md`
- `CLIENT_SHELL_API.md`
- `CAMPUS_COMMUNITIES_MY_API.md`
- `SOCIAL_FEED_API.md`
- `SOCIAL_CREATE_POST_API.md`
- `SOCIAL_STORIES_API.md`
- `SOCIAL_USER_SEARCH_API.md`
- `SOCIAL_FOLLOW_API.md`
- `SOCIAL_PUBLIC_PROFILE_API.md`
- `SOCIAL_VIBES_API.md`
- `RESOURCES_LIST_API.md`
- `RESOURCES_CREATE_API.md`
